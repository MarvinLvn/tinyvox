...
def match_files(data_path):
    print("Parsing CHAT files.")
    cha_files = list(data_path.glob('**/*.cha'))
    chat_files = [
    ...
    automatic_chat_files = [f for f in chat_files if f.is_automatic]
    chat_files = [f for f in chat_files if not f.is_automatic]
    automatic_filenames = [f.filename for f in automatic_chat_files]
    ...
    return matched, unmatched, automatic_filenames


def save_results(output_dir, matched, unmatched, automatic):
...
if __name__ == '__main__':
    DATA_PATH = Path('/scratch1/data/raw_data/CLEAR_ASR/IN_PREP')
    matched, unmatched, automatic = match_files(DATA_PATH)
    save_results('pairs', matched, unmatched, automatic)
    # Print summary
    print("\nResults Summary:")
