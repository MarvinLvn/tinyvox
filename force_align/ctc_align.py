...
# Originally from NVIDIA CORPORATION
# Modified by Marvin Lavechin
import argparse
import logging
import os
import sys
...
if __name__ == "__main__":
    ...
    if hasattr(asr_model, 'tokenizer_cfg') and len(asr_model.tokenizer_cfg['langs']) > 1:
        if not (data / 'languages.csv').exists():
            raise ValueError(f"Cannot find {data / 'languages.csv. This is needed for multilingual BPE-based ASR.'}")
        else:
            is_multilingual = True
            lang_data = pd.read_csv(data / 'languages.csv')
    ...
    if len(all_log_probs) > 0:
        ...
        normalized_lines = Parallel(n_jobs=args.num_jobs)(
            delayed(get_segments)(
                ...
                index_duration,
                args.window_len,
                lang=lang_data[str(path_audio.resolve())] if is_multilingual else None,
                log_file=log_file,
                debug=args.debug,
            )
